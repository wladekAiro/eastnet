...
*   */
            String sqlPendingOrders = "SELECT EXTRACT(DAY FROM ordered_On::date) AS OrderDate, SUM(total_order_price) AS price "
                    + " FROM  orders "
                    + " WHERE ("
                    + " EXTRACT(YEAR FROM ordered_On::date) = '" + year + "'"
                    + " ) "
                    + " AND "
                    + "("
                    + " EXTRACT(MONTH FROM ordered_On::date) ='" + month + "'"
                    + " ) "
                    + " AND ("
                    + " status =  'pending'"
                    + " )"
                    + " GROUP BY EXTRACT(DAY FROM ordered_On::date)";

            String sqlApprovedOrders = "SELECT EXTRACT(DAY FROM ordered_On::date ) AS OrderDate, SUM(  total_order_price ) AS price "
                    + " FROM  orders "
                    + " WHERE ("
                    + " EXTRACT(YEAR FROM ordered_On::date) = '" + year + "'"
                    + " ) "
            ...
            String sqlDelivredOrders = "SELECT EXTRACT(DAY FROM ordered_On::date) AS OrderDate, SUM(total_order_price) AS price "
                    + " FROM  orders "
                    + " WHERE ("
                    + " EXTRACT(YEAR FROM ordered_On::date) = '" + year + "'"
                    + " ) "
            ...
            String sqlExpenses = "SELECT "
                    + " EXTRACT(DAY FROM purchase_date::date) AS purchaseDate, SUM(price) AS expense "
                    + " FROM  expenses "
                    + " WHERE  (EXTRACT(YEAR FROM purchase_date::date) = '" + year + "') "
                    + " AND ( EXTRACT(MONTH FROM purchase_date::date) = '" + month + "') "
